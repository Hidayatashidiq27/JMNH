...
// Memanggil Gemini di SISI SERVER agar GEMINI_API_KEY tidak pernah terkirim ke browser.
// Key diambil dari environment variable rahasia di Cloudflare (context.env.GEMINI_API_KEY).
...
export const onRequestPost = async (context: any): Promise<Response> => {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (res.ok) {
      ...
      // Teruskan JSON hasil Gemini apa adanya ke frontend.
